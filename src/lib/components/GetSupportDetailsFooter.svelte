<script>
    ...
    const articleCopy = getSelectedArticle();

    // Function to be called when the component is mounted
    onMount(() => {
        if (articleCopy) {
            console.log('articleCopy from onMount is:', articleCopy);
            // Set articleSteps and update mountedArticleSteps store
            articleSteps = articleCopy.steps;
            // console.log('articleSteps', articleSteps);
            mountedArticleSteps.set(articleCopy.steps);

            // Combine objective and applicable devices into a single string
            const objective = articleCopy.objective;
            const applicableDevices = articleCopy.applicable_devices
                .map(
                    (device) =>
    ...
    async function performApiCall(
        ...
        console.log('mostRecentStep is: ', $mostRecentStep);
        if ($mostRecentStep === undefined) {
            console.log('article from performAPI Call / if statement: ', article);
        }
        ...
        // messageWell
        // const messageWellSetHeight = document.querySelector('.messageWell');
        // messageWellSetHeight.style.height = 'auto';
        try {
            isLoading = true;
    ...
    async function apiCallOne(message, buttonText) {
        userClickedButton = "I don't understand this article";
        console.log(
            'From ApiCallOne in GetSupportDetailsFooter, mostRecentStep is: ',
            $mostRecentStep
        );
        const stepArrayContext =
            $mostRecentStep && $mountedArticleSteps.length > 0
    ...
    export async function apiCallFour(message, buttonText) {
        console.log('$mostRecentStep:', $mostRecentStep);
        console.log('$mountedArticleSteps:', $mountedArticleSteps);

        const stepArrayContext = $mountedArticleSteps;
...
<div class="detailsContainer">
    ...
    >
        <summary tabindex="0" on:click={handleClick()}>
            <div class="summaryMobile">
                ...
                <p class="getSupportHeader">Get Support</p>
            </div></summary
        >
        ...
        {#if questionsFetched}
            <details
                ...
                id="detailsFaqlike"
                style="margin:1em 0 0 0 ;border-left:none;"
                open="true"
                in:fly={{ y: 25, duration: 1000 }}
            >
